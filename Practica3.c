...
double funcion(double x){
    return tan(x *  M_PI/180.0); //f(x) = tan(x)
}

//prototipos
double trapecios(unsigned long, double, double);
double simpson(unsigned long, double, double);

/// @brief Funcion main
/// @return 0
int main(){
    double a = 0.0, b = 0.0; //limites
    int p = 0; //potencia de intervalos
...
/// @brief calcula una integral con trapecios
/// @param n iteraciones
/// @param a limite inferior
/// @param b limite superior
/// @return resultado de la integracion
double trapecios(unsigned long n, double a, double b) {
    double h = (b - a) / n;
    double suma = 0.0;

    for (int i = 1; i < n; i++) {
        suma += funcion(a + i * h); //suma secuencial
    }

    suma += (funcion(a) + funcion(b)) / 2.0;
    return h * suma;
}

/// @brief calcula una integral con el metodo de simpson
/// @param n iteraciones
/// @param a limite inferior
/// @param b limite superior
/// @return resultado de la integracion
double simpson(unsigned long n, double a, double b) {
    ...
    for (int i = 1; i < n; i++) {
        if (i % 2 == 0) {
            suma += 2 * funcion(a + i * h); //suma secuencial
        } else {
            suma += 4 * funcion(a + i * h); //suma secuencial
        }
    }
